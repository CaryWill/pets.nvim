...
function M.Animation:stop()
  if self.current_image then
    self.current_image:clear(0, { free = false })
  end
  self:stop_timer()
end
...
function M.Animation:next_frame()
  ...
  --   x = self.col,
  --   -- y = self.row,
  --   y = vim.api.nvim_win_get_height(0) - 2
  -- })
  local y_position = vim.api.nvim_win_get_height(0) - 2
  local params = {
    x = self.col,
    y = y_position
  }
  ...
  -- If moving fails, render instead
  if not success then
    -- Optionally, handle the error message in `result` here
    -- Call render as fallback
    -- if it's a rendered image then call move
    -- or the cursor will blink crazily fast
    image:render(params)
  end
  -- local ok = pcall(image.display)
  -- if not ok then
  --   self:stop()
  --   if self.popup then
  --     self.popup:unmount()
